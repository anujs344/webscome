        ...
        <div class="invoice">
            ...
            <div class="invoice-company text-inverse f-w-600">
                    <span class="pull-right hidden-print">
                        <!--<a href="{{ route('download_pdf',[$order[0]->order_id]) }}"-->
            ...
            <div class="invoice-header">
                ...
                <div class="invoice-to">
                    ...
                    <address class="m-t-5 m-b-5">
                        <strong class="text-inverse text-capitalize">{{ $order[0]->user_name }}</strong><br>
                        Email: {{ $order[0]->user_email }}<br>
                        {{ $order[0]->order_address }} ,<br>

                        {{ $order[0]->order_city }}<br>
                        {{ $order[0]->order_pincode }}<br>
                ...
                <div class="table-responsive">
                    <table class="table table-invoice">
                        ...
                        <tr>
                            ...
                            <th class="text-left">Spares Added</th>
                            <th class="text-left" width="10%">Subtotal/Qty</th>
                        ...
                        <?php $subtotal = 0; ?>
                        @foreach($order as $o)
                            <?php $subtotal += ($o->discounted_price*$o->service_quantity)?>
                            <tr>
                                <td>
                                    <span class="text-inverse">{{ $o->service }}</span><br>

                                </td>
                                <td class="text-left">{{'#'.str_pad($o->order_id, 8, "0", STR_PAD_LEFT)}}</td>
                                ...
                                <td>
                                    ...
                                    $spares= DB::table('added_spares')
                                        ->join('spare_parts','added_spares.spare_id','=','spare_parts.id')
                                        ->where('added_spares.order_id','=',$order_id)
                                        ->where('added_spares.service_id','=',$service_id)
                                        ->select('added_spares.*','spare_parts.spare_part_name')
                                        ->get();
                                    ...
                                    @foreach($spares as $spare)
                                        <strong>{{$spare->spare_part_name }}</strong> - ₹{{$spare->spare_price}}<br>
                                        <?php $subtotal+=$spare->spare_price; ?>
                                    @endforeach
                                </td>
                                ...
                                <td class="text-left">₹{{ $o->discounted_price }}
                                    <small>
                                        <del>{{$o->regular_price}}</del>
                                    </small>
                                </td>
                                <td class="text-left">{{$o->service_quantity}}</td>
                                <td class="text-left">₹{{ ($o->regular_price-$o->discounted_price)*$o->service_quantity }}</td>
                                <td class="text-left">
                                    <?php $ser_total=0;
                                    $ser_total+= $o->discounted_price * $o->service_quantity;
                                    ?>
                                    @foreach($spares as $spare)
                                        <?php $ser_total+=$spare->spare_price; ?>
                                    @endforeach
                                    {{$ser_total}}
                                </td>
                                </tr>
                ...
                <div class="invoice-price">
                    <div class="invoice-price-left">
                        <div class="invoice-price-row">
                            ...
                            @if($order[0]->applied_promo_code != null)
                                <div class="sub-price">
                                    <i class="fa fa-minus text-muted"></i>
                                </div>
                                <div class="sub-price">
                                    <small>Promo Discount <strong>({{$order[0]->applied_promo_code}})</strong></small>
                                    <span class="text-inverse">₹{{ $order[0]->promo_discount }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                    <div class="invoice-price-right">
                        <small>TOTAL</small> <span class="f-w-600">
                            <?php $promo_dis= $order[0]->applied_promo_code == null ? 0: $order[0]->promo_discount ?>
                            ₹{{ $subtotal - $promo_dis}}
                        </span>
                    </div>
                </div>
